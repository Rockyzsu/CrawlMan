...
while True:
    ...
    for repo in repo_array:
        if not repo['fork']:
            repo_list.append([repo['name'], repo['stargazers_count'], repo['forks_count'],'' if repo['description'] is None else repo['description']])
    page_id += 1
...
print('='*55)
print('{:<30}\t{:<30}'.format('total_repo_count',len(repo_list)))
